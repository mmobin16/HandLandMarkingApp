...
class HandLandmarkerHelper(
    ...
    fun detectVideoFile(
        ...
        try {
            ...
            val frameCount = (videoLengthMs / inferenceIntervalMs).toInt()
            ...
            return ResultBundle(
                results = resultList.filterNotNull(),
                inferenceTime = if (frameCount > 0) inferenceTimeMs / frameCount else 0L,
                inputImageHeight = videoHeight,
                inputImageWidth = videoWidth
            )
        } catch (e: Exception) {
            handLandmarkerHelperListener?.onError("Video processing error: ${e.message}", OTHER_ERROR)
            return null
        } finally {
            retriever.release()
        }
    ...
    fun getDelegateName(delegate: Int): String {
        return when (delegate) {
            DELEGATE_GPU -> "GPU Accelerated"
            else -> "CPU (Default)"
        }
    }

    fun getConfidenceDescription(confidenceType: String): String {
        return when (confidenceType) {
            "Detection" -> "Minimum confidence for hand detection"
            "Tracking" -> "Minimum confidence for hand tracking"
            "Presence" -> "Minimum confidence for hand presence"
